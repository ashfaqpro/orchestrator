(function($) {
    $.fn.bjqs = function(options) {
        ...
        if (settings.current !== '') {
            bjqsGo(false, settings.current);
            return true;
        }

        // Make everything consistent in size
        // TODO: move away from px and make slider responsive
        //rootyb: Five-minute semi-responsive test 1! Works great, but doesn't resize (since it's actually setting the size based on pixels. Works for my needs, but probably nobody else's)
        if (typeof(settings.height) == "string" && settings.height.indexOf("%")) {
            var heightArray = settings.height.split("%");
            respHeight = heightArray[0] * $container.parent().height() / 100;
            settings.height = respHeight;
        }

        if (typeof(settings.width) == "string" && settings.width.indexOf("%")) {
            var widthArray = settings.width.split("%");
            respWidth = widthArray[0] * $container.parent().width() / 100;
            settings.width = respWidth;
        }
        ...
        // RootyB: Added this to bind a public method (function? I don't even know) to the plugin
